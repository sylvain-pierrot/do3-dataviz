...
ui <- fluidPage(
  titlePanel("Toutes les missions spatiales depuis le début de la course à l'espace (1957)"),
  sidebarLayout(
    sidebarPanel(
      selectInput(inputId = "org", label = "Choisir une organisation", choices = org_list),
      sliderInput(inputId = "year", label = "Choisir une année de lancement", 
            min = min(year(space_missions$launch_date)), 
            max = max(year(space_missions$launch_date)), 
            value = 2020, sep = "")
    ),
    mainPanel(
      tabsetPanel(
        tabPanel("Lancements par année",
                 plotOutput(outputId = "plot1")),
        tabPanel("Coûts et pertes",
                 plotOutput(outputId = "plot2")),
        tabPanel("Statut des missions",
                 plotOutput(outputId = "plot3")),
        tabPanel("Informations sur les fusées",
...
server <- function(input, output) {
  ...
  output$plot1 <- renderPlot({
    org_data() %>%
      filter(as.numeric(substr(launch_date, 1, 4)) >= input$year) %>% # filtrer 
  ...
  # Graphique des dépenses et pertes par pays
  output$plot2 <- renderPlot({
  space_missions %>%
    filter(as.numeric(substr(launch_date, 1, 4)) >= input$year) %>% # filtrer 
    mutate(cost_type = ifelse(status_mission == "Success", "Coût", "Perte")) %>%
    group_by(country, cost_type) %>%
    summarise(total_cost = sum(cost, na.rm = TRUE)) %>%
    arrange(total_cost) %>%
    ggplot(aes(x = reorder(country, total_cost), y = total_cost, fill = cost_type)) +
  ...
  # Graphique de la répartition des dates de lancement en fonction du statut de la mission
  output$plot3 <- renderPlot({
    space_missions %>%
      filter(as.numeric(substr(launch_date, 1, 4)) >= input$year) %>% # filtrer en fonction de l'année minimum sélectionnée
      ggplot(aes(x = status_mission, y = launch_date, fill = status_mission)) +
      geom_violin(scale = "count", trim = FALSE, alpha = 0.7, width = 1) +
      scale_fill_manual(values = c("#d91515", "#d97115", "#d9cd15", "#49d915"), 
                        labels = c("Échec", "Échec partiel", "Échec du pré-lancement", "Succès")) +
      scale_x_discrete(labels = c("Échec", "Échec partiel", "Échec du pré-lancement", "Succès")) +
  ...
  # Tableau avec les informations sur les missions de l'organisation sélectionnée
  output$table <- DT::renderDataTable({
    org_data() %>%
      select(rocket_name, launch_date, location, status_mission) %>%
      rename("Nom de la fusée" = rocket_name,
             "Date de lancement" = launch_date,
             "Site de lancement" = location,
             "Statut de la mission" = status_mission) %>%
  ...
  # Carte avec les sites de lancement de l'organisation sélectionnée
  #output$map <- renderLeaflet({
  #  org_data() %>%
  #    select(launch_site_name, launch_site_latitude, launch_site_longitude) %>%
  #    distinct() %>%
  #    leaflet() %>%
  #    addTiles() %>%
  #    addMarkers(lng = ~launch_site_longitude, lat = ~launch_site_latitude, popup = ~launch_site_name)
  #})
  }
